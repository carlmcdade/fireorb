...
# Originally written for FireOrb (http://fireorb.info/) to ensure stylish
...
while (<>) {
  ...
  # XHTML requires closing tag
    ...
    $msg = "the use of REQUEST_URI is prone to XSS exploits and does not work on IIS; use fo_1_request_uri() instead";
  }
  elsif (/\"REQUEST_URI\"/i) {
    $msg = "the use of REQUEST_URI is prone to XSS exploits and does not work on IIS; use fo_1_request_uri() instead";
  }
