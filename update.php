...
/**
 * @file
 * Administrative page for handling updates from one FireOrb version to another.
 *
...
function update_sql($sql) {
  $result = db_query($sql);
  return array('success' => $result !== FALSE, 'query' => fo_1_check_plain($sql));
}
...
/**
 * If the schema version for FireOrb core is stored in the variables table
...
function update_fix_schema_version() {
  if ($update_start = fo_1_variable_get('update_start', FALSE)) {
    ...
    switch ($update_start) {
      case '2005-04-14':
        fo_1_variable_set('update_132_done', TRUE);
        break;

      case '2005-05-06':
        fo_1_variable_set('update_132_done', TRUE);
        fo_1_variable_set('update_135_done', TRUE);
        break;

      case '2005-05-07':
        fo_1_variable_set('update_132_done', TRUE);
        fo_1_variable_set('update_135_done', TRUE);
        fo_1_variable_set('update_137_done', TRUE);
        break;

    }
    // The schema_version column (added below) was changed during 4.7beta.
    // Update_170 is only for those beta users.
    fo_1_variable_set('update_170_done', TRUE);

    $sql_updates = array(
      '2004-10-31: first update since FireOrb 4.5.0 release' => 110,
      ...
      '2005-04-08: first update since FireOrb 4.6.0 release' => 129,
    ...
    fo_1_set_installed_schema_version('system', $sql_updates[$update_start]);
    fo_1_variable_del('update_start');
  }
}
 ...
 * when update 130 is removed. It is part of the FireOrb 4.6 to 4.7 migration.
 */
function update_fix_sessions() {
  $ret = array();

  if (fo_1_get_installed_schema_version('system') < 130 && !fo_1_variable_get('update_sessions_fixed', FALSE)) {
    ...
    fo_1_variable_set('update_sessions_fixed', TRUE);
  }
}
 ...
 * when update 115 is removed. It is part of the FireOrb 4.5 to 4.7 migration.
 */
function update_fix_watchdog_115() {
  if (fo_1_get_installed_schema_version('system') < 115 && !fo_1_variable_get('update_watchdog_115_fixed', FALSE)) {
    if ($GLOBALS['db_type'] == 'mysql') {
    ...
    fo_1_variable_set('update_watchdog_115_fixed', TRUE);
  }
}
 ...
 * when update 142 is removed. It is part of the FireOrb 4.6 to 4.7 migration.
 */
function update_fix_fo_1_watchdog() {
  if (fo_1_get_installed_schema_version('system') < 142 && !fo_1_variable_get('update_watchdog_fixed', FALSE)) {
    switch ($GLOBALS['db_type']) {
      case 'pgsql':
        $ret = array();
    ...
    fo_1_variable_set('update_watchdog_fixed', TRUE);
  }
}
...
function update_data($module, $number) {
  ...
  if ($finished == 1) {
    ...
    fo_1_set_installed_schema_version($module, $number);
  }
...
function update_selection_page() {
  $output = '<p>The version of FireOrb you are updating from has been automatically detected. You can select a different version, but you should not need to.</p>';
  ...
  foreach (module_list() as $module) {
    $updates = fo_1_get_schema_versions($module);
    if ($updates !== FALSE) {
      $updates = fo_1_map_assoc($updates);
      $updates[] = 'No updates available';

      $form['start'][$module] = array(
        '#type' => 'select',
        '#title' => $module . ' module',
        '#default_value' => array_search(fo_1_get_installed_schema_version($module), $updates) + 1,
        '#options' => $updates,
      );
    }
  }
  ...
  fo_1_set_title('FireOrb database update');
  // Prevent browser from using cached fireorb.js or update.js
  fo_1_add_js('misc/update.js', TRUE);
  $output .= fo_1_get_form('update_script_selection_form', $form);
...
function update_update_page() {
  ...
  foreach ($_POST['edit']['start'] as $module => $version) {
    fo_1_set_installed_schema_version($module, $version - 1);
    $max_version = max(fo_1_get_schema_versions($module));
...
function update_progress_page() {
  // Prevent browser from using cached fireorb.js or update.js
  fo_1_add_js('misc/progress.js', TRUE);
  fo_1_add_js('misc/update.js', TRUE);

  fo_1_set_title('Updating');
...
function update_do_update_page() {
  ...
  if ($_SERVER['REQUEST_METHOD'] != 'POST') {
    fo_1_set_message('HTTP Post is required.', 'error');
    fo_1_set_title('Error');
    return '';
  }
  ...
  print fo_1_to_js(array('status' => TRUE, 'percentage' => $percentage, 'message' => $message));
}
...
function update_progress_page_nojs() {
  ...
  fo_1_set_html_head('<meta http-equiv="Refresh" content="0; URL=update.php?op='. $new_op .'">');
  $output = theme('progress_bar', $percentage, $message);
  $output .= '<p>Updating your site will take a few seconds.</p>';

  // Note: do not output fo_1_set_message()s until the summary page.
...
function update_finished_page($success) {
  fo_1_set_title('FireOrb database update');
  // NOTE: we can't use fo_1_link() here because the URL would point to 'update.php?q=admin'.
  $links[] = '<a href="'.  fo_1_base_bath() .'">main page</a>';
  $links[] = '<a href="'.  fo_1_base_bath() .'?q=admin">administration pages</a>';
...
function update_info_page() {
  fo_1_set_title('FireOrb database update');
  $output = "<ol>\n";
  $output .= "<li>Use this script to <strong>upgrade an existing FireOrb installation</strong>. You don't need this script when installing FireOrb from scratch.</li>";
  ...
  $output .= "<li>Update your FireOrb sources, check the notes below and <a href=\"update.php?op=selection\">run the database upgrade script</a>. Don't upgrade your database twice as it may cause problems.</li>\n";
  $output .= "<li>Go through the various administration pages to change the existing and new settings to your liking.</li>\n";
  $output .= "</ol>";
  $output .= '<p>For more help, see the <a href="http://fireorb.info/node/258">Installation and upgrading handbook</a>. If you are unsure what these terms mean you should probably contact your hosting provider.</p>';
  return $output;
}

function update_access_denied_page() {
  fo_1_set_title('Access denied');
...
<ol>
 <li>With a text editor find the update.php file on your system. It should be in the main FireOrb directory that you installed all the files into.</li>
...
// This code may be removed later.  It is part of the FireOrb 4.5 to 4.7 migration.
function update_fix_system_table() {
  fo_1_bootstrap(FO_BOOTSTRAP_DATABASE);
...
// This code may be removed later.  It is part of the FireOrb 4.6 to 4.7 migration.
function update_fix_access_table() {
  if (fo_1_variable_get('update_access_fixed', FALSE)) {
    return;
  }
  ...
  // Don't run again
  fo_1_variable_set('update_access_fixed', TRUE);
}
...
if (($access_check == FALSE) || ($user->uid == 1)) {
  ...
  switch ($op) {
    case 'Update':
      ...
      if (fo_1_valid_token($_REQUEST['edit']['form_token'], 'update_script_selection_form', TRUE)) {
        $output = update_update_page();
      }
      else {
        form_set_error('form_token', fo_1_text('Validation error, please try again.  If this error persists, please contact the site administrator.'));
